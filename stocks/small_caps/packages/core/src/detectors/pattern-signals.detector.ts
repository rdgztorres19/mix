import type { MarketContext, PatternResult } from '@small-caps/shared';
...
/** Facade over all individual pattern detectors. */
export class PatternSignalsDetector {
  ...
  detect(ctx: MarketContext): PatternDetectionResult {
    ...
    if (bullFlagResult.detected) { patterns.push(bullFlagResult); signals.push(bullFlagResult.description); }
    if (abcdResult.detected) { patterns.push(abcdResult); signals.push(abcdResult.description); }
    if (orbResult.detected) { patterns.push(orbResult); signals.push(orbResult.description); }
    if (vwapReversalResult.detected) { patterns.push(vwapReversalResult); signals.push(vwapReversalResult.description); }
    if (fallenAngelResult.detected) { patterns.push(fallenAngelResult); signals.push(fallenAngelResult.description); }
