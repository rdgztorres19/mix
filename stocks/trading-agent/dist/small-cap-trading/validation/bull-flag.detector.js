...
Object.defineProperty(exports, "BullFlagDetector", {
    enumerable: true,
    get: function() {
        return BullFlagDetector;
    }
});
let BullFlagDetector = class BullFlagDetector {
    detect(candles) {
        const empty = {
            detected: false,
            name: 'BULL_FLAG',
                    ...
                    anchor_points: [
                        {
                            label: 'PoleStart',
                            price: poleStart,
                            time: poleCandles[0].t
                        },
                        {
                            label: 'PoleHigh',
                            price: poleHigh,
                            time: poleCandles[poleCandles.length - 1].t
                        },
                        {
                            label: 'FlagLow',
                            price: flagLow,
                            time: flagCandles[flagCandles.length - 1].t
                        }
                    ],
    ...
    /** Pole: mostly green candles with net upward move. At least 70% must be green. */ isValidPole(candles) {
    ...
    /** Flag: consolidation — at least half candles red or small-bodied (doji). */ isValidFlag(flagCandles) {
