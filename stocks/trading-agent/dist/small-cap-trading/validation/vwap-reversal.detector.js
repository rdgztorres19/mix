...
Object.defineProperty(exports, "VwapReversalDetector", {
    enumerable: true,
    get: function() {
        return VwapReversalDetector;
    }
});
let VwapReversalDetector = class VwapReversalDetector {
    detect(candles, vwap) {
        const empty = {
            detected: false,
            name: 'VWAP_REVERSAL',
    ...
    /** Below VWAP: failure to make new low + HH/HL pattern = bullish reversal. */ detectBullishReversal(candles, vwap) {
        ...
        return {
            ...
            anchor_points: [
                ...
                },
                {
                    label: 'VWAP',
                    price: vwap,
                    time: recent[recent.length - 1].t
                },
                {
                    label: 'CurrentPrice',
                    price: recent[recent.length - 1].c,
                    time: recent[recent.length - 1].t
                }
            ],
    ...
    /** Above VWAP: failure to make new high = bearish reversal. */ detectBearishReversal(candles, vwap) {
        const recent = candles.slice(-5);
        if (recent.length < 3) {
            return {
                detected: false,
