...
let ScannerService = ScannerService_1 = class ScannerService {
    constructor() {
        ...
        this.logger.log(`ScannerService initialized → ${this.momoBase} | ` +
    ...
    async getStockSnapshotFromMomo(ticker, cutoffMs, timeframe = '5m') {
        ...
        this.logger.log(`Fetching snapshot for ${ticker} | ${timeframe}` +
        ...
        const prior_close = priorDayCandles.length > 0
        ...
        const atr = small_cap_trading_1.AtrCalculator.calculate(candlesForMetrics, this.ATR_PERIOD);
        return {
            ticker,
            price,
            vwap,
    ...
    evaluateSnapshot(snap) {
        ...
        if (price < this.FILTERS.price.min || price > this.FILTERS.price.max)
            return null;
        if (change_pct < this.FILTERS.change_pct.min)
            return null;
        if (atr < this.FILTERS.atr.min)
            return null;
        if (relative_volume < this.FILTERS.relative_volume.min)
            return null;
    ...
    async getDefaultWatchlist() {
        try {
            const res = await axios_1.default.get(`${this.momoBase}/screener`, { timeout: 8000 });
