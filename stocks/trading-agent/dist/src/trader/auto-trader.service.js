...
let AutoTraderService = AutoTraderService_1 = class AutoTraderService {
    constructor(predictor, alpaca, positions, mysqlRepo) {
    ...
    async shouldEnterTrade(result, row) {
        if (!result.tradeable || !this.tradeEnabled || !this.alpaca.isEnabled())
            return false;
        if (!this.isBeforeNoonET(row.candle_time_et))
            return false;
        return this.mysqlRepo.passesPrefilterForToday(row.symbol);
    }
    isBeforeNoonET(candleTimeEt) {
