...
let AlpacaTraderService = class AlpacaTraderService {
    ...
    async buyBracketLimit(symbol, dollarAmount, lastPrice, options = {}) {
        ...
        this.logger.log(`BUY BRACKET ${symbolUpper} notional=$${dollarAmount.toFixed(2)} ` + `entry=${payload.limit_price} tp=${payload.take_profit.limit_price} ` + `sl=${payload.stop_loss.stop_price} qty=${qty} tif=${timeInForce}`);
            ...
            try {
                ...
                if (order.status === 'partially_filled') {
                    ...
                    if (position && position.qty > 0) {
                        this.logger.warn(`Open position remains after partial fill: ${symbol} qty=${position.qty}. ` + `Decide whether to close it or re-protect it.`);
                    }
                    return;
                }
