...
Object.defineProperty(exports, "ScreenerRepository", {
    enumerable: true,
    get: function() {
...
let ScreenerRepository = class ScreenerRepository {
    getPool() {
    ...
    async ensureTables() {
      ...
      `);
            try {
                const [cols] = await p.query("SHOW COLUMNS FROM screener_active_symbols LIKE 'session_date'");
                if (!cols.length) {
                    ...
                    await p.query(`CREATE INDEX idx_screener_active_symbols_session_date ON screener_active_symbols (session_date)`);
                }
            } catch (e) {
                this.logger.warn(`screener_active_symbols migration failed: ${e.message}`);
            }
            await p.query(`
    ...
    async getPrevCloseMapForDate(asOfDate) {
        ...
        if (!p) return m;
        try {
            const [rows] = await p.query('SELECT symbol, prev_close FROM screener_prev_close WHERE as_of_date = ?', [
                asOfDate
            ]);
            for (const r of rows){
    ...
    async getPrevClose(symbol, asOfDate) {
        ...
        if (!p) return null;
        try {
            const [rows] = await p.query('SELECT prev_close FROM screener_prev_close WHERE symbol = ? AND as_of_date = ?', [
                symbol.toUpperCase(),
                asOfDate
            ]);
    ...
    async upsertPrevClose(symbol, asOfDate, prevClose, source) {
        await this.upsertPrevClosesBatch(asOfDate, source, [
            {
                symbol,
                prevClose
            }
    ...
    async replaceRankRows(rankType, rows) {
        ...
        try {
            await conn.beginTransaction();
            await conn.query('DELETE FROM screener_rank_rows WHERE rank_type = ?', [
                rankType
            ]);
            const batchSize = this.dbBatchSize();
            const sqlPrefix = `
    ...
    async replaceActiveSymbols(entries, sessionDate) {
        const p = this.getPool();
        if (!p) return;
        const conn = await p.getConnection();
        try {
    ...
    async getActiveSymbols(sessionDate) {
        const p = this.getPool();
        if (!p) return [];
        try {
            const [rows] = await p.query('SELECT rank_order, symbol, score FROM screener_active_symbols WHERE session_date = ? ORDER BY rank_order ASC', [
                sessionDate
            ]);
            return rows.map((r)=>({
                    rank_order: Number(r.rank_order),
    ...
    async updateRunMeta(sessionDate, symbolsScanned, note) {
        const p = this.getPool();
        if (!p) return;
        await p.query(`UPDATE screener_run_meta SET last_run_utc = UTC_TIMESTAMP(), last_session_date = ?, symbols_scanned = ?, note = ? WHERE id = 1`, [
            sessionDate,
            symbolsScanned,
            note ?? null
        ]);
    ...
    constructor(){
        this.logger = new _common.Logger(ScreenerRepository.name);
        this.pool = null;
    }
};
ScreenerRepository = _ts_decorate([
