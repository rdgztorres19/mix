...
Object.defineProperty(exports, "AlpacaDataSource", {
    enumerable: true,
    get: function() {
...
let AlpacaDataSource = class AlpacaDataSource {
    async getStockSnapshot(ticker, options) {
        ...
        try {
            ...
            this.logger.log(`🎯 Fetching Alpaca snapshot for ${ticker} | ${timeframe}` + (cutoffMs ? ` [SIMULATION cutoff: ${new Date(cutoffMs).toLocaleString('en-US', {
            ...
            const vwap = _smallcaptrading.VwapCalculator.calculate(candlesForMetrics);
            const vwap_line = _smallcaptrading.VwapCalculator.calculateLine(candlesForMetrics);
            const closes = candlesForMetrics.map((c)=>c.c);
            const ema9 = _smallcaptrading.EmaCalculator.calculate(closes, 9);
            const ema20 = _smallcaptrading.EmaCalculator.calculate(closes, 20);
            const atr = _smallcaptrading.AtrCalculator.calculate(candlesForMetrics, 14);
    ...
    async fetchBarWithRetries(ticker, dateStr, attempt = 1) {
        ...
        try {
            ...
            const errMsg = err instanceof _axios.AxiosError ? err.message : String(err);
   ...
   */ async fetchBarsFromAlpacaDirect(params) {
        ...
        try {
            ...
            const errMsg = error instanceof _axios.AxiosError ? error.message : String(error);
   ...
   */ async fetch1mBarsForDate(symbol, dateStr) {
    ...
    constructor(configService){
        ...
        this.logger = new _common.Logger(AlpacaDataSource.name);
        ...
        this.cache = new Map();
        this.alpacaKeyId = configService.get('ALPACA_KEY_ID', 'AKAS3FTVF54TKVHQSOO44I5XJH');
        this.alpacaSecretKey = configService.get('ALPACA_SECRET_KEY', 'Br5quiybxDxEhw2WsX1EhHMq83f4TZX4RAhoxzkdQG2d');
        this.logger.log('✅ AlpacaDataSource initialized with premium SIP feed (MoMo fallback disabled)');
    }
};
AlpacaDataSource = _ts_decorate([
