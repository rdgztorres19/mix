...
let ScannerService = class ScannerService {
   ...
   */ async getStockSnapshot(ticker, cutoffMs, timeframe = '5m') {
   ...
   */ async getStockSnapshotFromMomo(ticker, cutoffMs, timeframe = '5m') {
        ticker = ticker.toUpperCase();
        this.logger.log(`Fetching snapshot for ${ticker} | ${timeframe}` + (cutoffMs ? ` [SIMULATION cutoff: ${new Date(cutoffMs).toLocaleString('en-US', {
        ...
        const latest = candlesForMetrics.length ? candlesForMetrics[candlesForMetrics.length - 1] : candles1mDay[candles1mDay.length - 1] ?? candles1m[candles1m.length - 1];
        ...
        const atr = _smallcaptrading.AtrCalculator.calculate(candlesForMetrics, this.ATR_PERIOD);
        return {
            ticker,
            price,
            vwap,
