...
Object.defineProperty(exports, "FastPipeline", {
    enumerable: true,
    get: function() {
        return FastPipeline;
    }
...
const _newscacheservice = require("../../cache/news-cache.service");
...
let FastPipeline = class FastPipeline {
    async run(req) {
        ...
        const [snap, cachedCatalyst, rawHeadlines] = await Promise.all([
            ...
            (0, _newstool.fetchYahooNews)(tickerUpper).then((h)=>h.length ? h : (0, _newstool.fetchFinvizNews)(tickerUpper))
        ]);
        ...
        if (catalyst.is_dilutive) {
            ...
            return this.earlyExitNoTrade({
                ...
                alertas: [
                    'Dilución confirmada — evitar longs',
                    'Posible setup short en rebote a VWAP',
    ...
    getSession(cutoffMs) {
        ...
        return (0, _scannertool.getSession)(etTime);
    }
    ...
    async resolveCatalyst(cached, rawHeadlines, tickerUpper, elapsed) {
        ...
        const catalyst = await (0, _newstool.scoreHeadlines)(rawHeadlines);
        ...
        return {
            catalyst,
            headlinesForDisplay: rawHeadlines
        };
    }
    applyRules(snap, session, timeframe, account_size, tickerUpper) {
        ...
        return (0, _smallcaptrading.applyTradingRules)({
            ticker: tickerUpper,
            price: snap.price,
            vwap: snap.vwap,
            ema9: snap.ema9,
    ...
    async runLlmAndRespond(params) {
        ...
        const stockText = (0, _scannertool.formatStockSnapshotForLLM)(snap, tickerUpper, timeframe, cutoff_ms);
        const newsText = headlinesForDisplay.slice(0, 6).map((h)=>{
...
${ragResult}
        ...
        const llmResponse = await this.llm.invoke([
            ...
            new _messages.HumanMessage(`Analyze ${tickerUpper}. Account: $${account_size.toLocaleString()}.\n\n${contextPrompt}`)
        ]);
    ...
    constructor(scannerService, newsCache, ragService, analysisLog, responseBuilder, llm){
        ...
        this.llm = llm;
        this.logger = new _common.Logger(FastPipeline.name);
    }
};
FastPipeline = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
        typeof _scannerservice.ScannerService === "undefined" ? Object : _scannerservice.ScannerService,
        typeof _newscacheservice.NewsCacheService === "undefined" ? Object : _newscacheservice.NewsCacheService,
