...
NONE|Routine / no clear catalyst`;
    ...
    return {
        strength: 'MODERATE',
        catalyst_type: 'General news (AI fallback)'
    };
}
/** Obtiene la última noticia de hoy (o la más reciente si no hay de hoy). */ function getLatestHeadline(headlines) {
...
function createNewsTool() {
    return (0, _tools.tool)(async ({ ticker })=>{
        ...
        const evaluatedLine = latestH ? `Headline evaluada (última de hoy): "${latestH.title.slice(0, 80)}${latestH.title.length > 80 ? '…' : ''}"` : '';
