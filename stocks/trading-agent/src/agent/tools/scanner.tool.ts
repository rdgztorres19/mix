  ...
  return tool(
    async ({ ticker }) => {
      try {
        ...
        const ema9Relation =
          snap.ema9
            ? snap.price > snap.ema9
              ? `above EMA9 (${snap.ema9.toFixed(2)})`
              : `below EMA9 (${snap.ema9.toFixed(2)})`
            : 'EMA9 unavailable';

        const ema20Relation =
          snap.ema20
            ? snap.price > snap.ema20
              ? `above EMA20 (${snap.ema20.toFixed(2)})`
        ...
        return `STOCK DATA: ${ticker.toUpperCase()}${simLabel}
...
function getSession(etTime: string): string {
