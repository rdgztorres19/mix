...
const CONFIG = {
  ...
  stockDayMode: "first_open_or_regular", // "first_open_or_regular" | "first_regular" | "first_seen"
  openSessions: new Set(["THE_OPEN", "REGULAR", "REGULAR_SESSION"]),
  includePremarketAsFallback: true,
...
function buildStockDayDataset(rows) {
  ...
  for (const [, arr] of grouped.entries()) {
    ...
    const target = computeStockDayTarget(baseRow, futureRows);
  ...
  log(`Stock-days únicos: ${grouped.size}`);
  log(`Filas base finales: ${out.length}`);
  log(`Stock-days sin base usable: ${noBase}`);
...
function buildCandidateConditions(rows) {
  ...
  log(`Condiciones candidatas finales: ${finalConds.length}`);
  return finalConds;
}
...
function searchPairRules(rows, baseline, seedConditions) {
  ...
  for (let i = 0; i < seedConditions.length; i++) {
    for (let j = i + 1; j < seedConditions.length; j++) {
      ...
      tested++;
      if (tested % 100 === 0) log(`Reglas dobles probadas: ${tested}`);
  ...
  results.sort((a, b) => b.score - a.score);
  log(`Reglas dobles válidas: ${results.length}`);
...
function generatePrefilterFile(bestRule) {
  ...
  bestRule.conditions.forEach((c, idx) => {
    ...
    if (c.type === "lte") expr = `Number.isFinite(row.${c.feature}) && row.${c.feature} <= ${c.value}`;
  ...
  body.push("}");
  body.push("");
  body.push("module.exports = { passesGeneratedPreFilter };");
  return body.join("\n");
}
...
function main() {
  ...
  writeJson("stockday_single.json", singleRules);
  writeJson("stockday_pair.json", pairRules);
  writeJson("stockday_baseline.json", baseline);
  ...
  printRules("TOP PAIR RULES STOCK-DAY", pairRules, 12);

  log("Archivos generados:");
  log("- stockday_single.json");
  log("- stockday_pair.json");
  log("- stockday_baseline.json");
