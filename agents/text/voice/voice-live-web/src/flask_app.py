#pip3 install "azure-ai-voicelive[aiohttp]" --trusted-host pypi.org --trusted-host files.pythonhosted.org
            ...
            async for msg in ws:
                if msg.type == web.WSMsgType.BINARY and assistant_instance and assistant_loop:
                    ...
                    if chunk_count <= 3 or chunk_count % 50 == 0:
                        _broadcast({"type": "log", "level": "debug", "msg": f"WS audio chunk #{chunk_count}, size={len(msg.data)} bytes"})
                    ...
                    _broadcast({"type": "log", "level": "warn", "msg": f"WS audio received but assistant not ready: instance={assistant_instance is not None}, loop={assistant_loop is not None}"})
                elif msg.type == web.WSMsgType.ERROR:
                    break
        except Exception as e:
            _broadcast({"type": "log", "level": "error", "msg": f"WS audio handler error: {e}"})
        finally:
            await ws.close()
...
class BasicVoiceAssistant:
    ...
    def __init__(self, endpoint, credential, model, voice, instructions, state_callback):
    ...
    async def start(self):
        ...
        self.state_callback("starting", "Connecting to VoiceLive…")
    ...
    async def _handle_session_updated(self):
        """Session configured successfully, ready for voice input."""
        self.state_callback("ready", "Session configured. You can start speaking.")
        _broadcast({"type": "log", "level": "info", "msg": "VoiceLive session configured successfully."})

    async def _handle_speech_started(self, conn):
        """User started speaking - cancel any ongoing assistant response."""
        self._response_cancelled = True
        _broadcast({"type": "control", "action": "stop_playback"})
        self.state_callback("listening", "Listening…")
        try:
